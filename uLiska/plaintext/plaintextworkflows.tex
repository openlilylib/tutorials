...
It provides professional typesetting as opposed to mere text processing, because an ordinary document author shouldn't have to know too much about the rules of typography.
...
Therefore I won't get into details introducing you to the concrete syntax of its input files.

\LaTeX{} is available in form of \emph{distributions} such as TeXLive for Linux, MacTeXfor Mac OS, or MiKTeX for Windows.
These distributions contain several core programs that actually perform the compilation as well as documentation and numerous extension \emph{packages}.

\begin{itemize}
\item As short as possible
\item short introduction about architecture (what is my interface to it?)
\item No syntax overview (just: somewhat similar to LilyPond)
\item Extension packages, includes
