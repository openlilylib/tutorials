...
You can also print full-page examples, with special support for examples starting on odd or even pages (you can force example to start at the next odd or even page).
...
There are tools that compile your LilyPond files correctly to files that can directly be used in the \LaTeX{} document.
This may be the way for you if you want to have your scores separate.
...
\emph{OOoLilyPond} is an OpenOffice extension which allows to store LilyPond code as embedded objects in Writer documents.
...
Unfortunately this can only produce raster images and no vector graphics, resulting in a rather medium output quality.
...
The “project” may be supported by extensive programming without the individual contributor knowing anything about it---well it may be explicitely programmed to expose the individual to a quite simple interface.
